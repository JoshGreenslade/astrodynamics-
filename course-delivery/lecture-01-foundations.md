# Lecture 01: Foundations Kickoff

Canonical lecture content path:
- `/course-delivery/module-01-foundations/chapter-01-introduction-to-two-body-dynamics/lecture-01-newtonian-gravity-and-two-body-assumptions.md`

## Date
2026-06-14

## Status
Taught

## Learning objectives
- Re-activate prerequisite vector and unit analysis skills.
- Introduce Newtonian gravitation as the basis for astrodynamics.
- Set up the two-body assumptions used throughout Stage 1.

## Lecture outline
1. Activation task: unit consistency and vector decomposition warm-up.
2. Worked derivation: inverse-square gravitational force and acceleration form.
3. Guided example: convert a state description into scalar/vector components.
4. Independent practice: short conic-geometry concept check.

## Formative assessment (exit ticket)
- Prompt 1: State the two-body assumptions and one practical limitation.
- Prompt 2: Compute gravitational acceleration magnitude at a given radius.
- Success criteria: correct setup, unit consistency, and interpretation.

## Delivery record (single student)
- Student: JoshGreenslade
- Delivery completed: 2026-06-14
- Observation notes:
  - Correctly stated the closed two-body assumptions and identified perturbations as the main limitation.
  - Set up gravitational acceleration with correct inverse-square form and units.
  - Needed one prompt to keep vector direction/sign conventions explicit.

## Exit ticket result
- Prompt 1: Met
- Prompt 2: Met with minor coaching on sign convention notation
- Overall result: Completed

## Follow-up remediation plan
- Start next lecture with a 5-minute vector-direction refresh before new derivations.
